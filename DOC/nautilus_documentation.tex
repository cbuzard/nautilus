...
The exact spatial points are of no importance because we will interpolate linearly to reconstruct the values at the spatial points defined in the code. 

The dust temperature is ruled by the type you define in \gras[parameter!grain\_temperature\_type]{grain\_temperature\_type} (see \refsec{sec:dust_temp} for more details).
...
The file \gras{.nautilus\_profile} in the Git repository contains Bash commands to give user friendly tools to use \gras{Nautilus} in command line. By running \gras{configure\_nautilus.py}, a call to this file will be done in the \gras{.bash\_profile}.
